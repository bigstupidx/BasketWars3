...
public abstract class UITweener : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if ((style == Style.Once) && (duration == 0f || mFactor > 1f || mFactor < 0f))
		{
			...
			if (current == null)
			{
				...
				if(current.direction == AnimationOrTween.Direction.Forward){
					if (onFinishedForward != null)
					{
						mTemp = onFinishedForward;
				...
				if(current.direction == AnimationOrTween.Direction.Reverse){
					if (onFinishedReverse != null)
					{
						mTemp = onFinishedReverse;
	...
	public void SetOnFinishedForward (EventDelegate.Callback del) { EventDelegate.Set(onFinishedForward, del); }

	/// <summary>
	/// Convenience function -- set a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>

	public void SetOnFinishedForward (EventDelegate del) { EventDelegate.Set(onFinishedForward, del); }

	/// <summary>
	/// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>

	public void AddOnFinishedForward (EventDelegate.Callback del) { EventDelegate.Add(onFinishedForward, del); }

	/// <summary>
	/// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>

	public void AddOnFinishedForward (EventDelegate del) { EventDelegate.Add(onFinishedForward, del); }
	...
	public void RemoveOnFinishedForward (EventDelegate del)
	{
		if (onFinishedForward != null) onFinishedForward.Remove(del);
	...
	public void SetOnFinishedReverse (EventDelegate.Callback del) { EventDelegate.Set(onFinishedReverse, del); }
	
	/// <summary>
	/// Convenience function -- set a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>
	
	public void SetOnFinishedReverse (EventDelegate del) { EventDelegate.Set(onFinishedReverse, del); }
	
	/// <summary>
	/// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>
	
	public void AddOnFinishedReverse (EventDelegate.Callback del) { EventDelegate.Add(onFinishedReverse, del); }
	
	/// <summary>
	/// Convenience function -- add a new OnFinished event delegate (here for to be consistent with RemoveOnFinished).
	/// </summary>
	
	public void AddOnFinishedReverse (EventDelegate del) { EventDelegate.Add(onFinishedReverse, del); }
	
	/// <summary>
	/// Remove an OnFinished delegate. Will work even while iterating through the list when the tweener has finished its operation.
	/// </summary>
	
	public void RemoveOnFinishedReverse (EventDelegate del)
	{
		if (onFinishedReverse != null) onFinishedReverse.Remove(del);
		if (mTemp != null) mTemp.Remove(del);
	}
