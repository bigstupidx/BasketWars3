...
public class StageButton : MonoBehaviour 
{
	...
	public void OnClick () {
		...
		else if (GameManager.s_Inst.m_current_game_state == GameManager.GameState.MainMenu && stageLevel > 11){ //bonus stages
	...
	public void SetStars()
	{
		...
		if(m_stars != null)
		{
			...
			int stars = GameManager.s_Inst.gameObject.GetComponent<StarManager>().GetStars(stage_name);
			if(stars >= 0){ // Stupid fix to stop map init calls before kii is init, breaking the WHOLE GAME
				if(stageLevel <= 11){
					if(stars == 0){
