...
public class StageUnlocker : MonoBehaviour 
{
	...
	//List of level buttons. To change unlock order. just reorder levels in the editor
	/// <summary>
	/// Holds reference to all the level select buttons.
	/// </summary>
	public int m_overworld_stage = 1;
	public int m_britain_current_stage = 2;
	...
	public void Init(int britain, int stalingrad, int kursk, int normandy){
		...
		if(m_britain_star_count[10] > 0){
			...
			m_stalingrad_button.transform.GetChild(0).GetComponent<Collider>().enabled = true;
		...
		if(GameManager.s_Inst.m_more_levels_unlocked){
			if(m_stalingrad_star_count[10] > 0){
				...
				m_kursk_button.transform.GetChild(0).GetComponent<Collider>().enabled = true;
		...
		if(m_highest_level == "Britain"){
			if(m_britain_current_stage < 10){
				m_highest_level += "0" + m_britain_current_stage;
			}else{
				m_highest_level += m_britain_current_stage;
			}
		}
	...
	public void UnlockNextLevel(string levelname, int stage){
		...
		if(levelname.Contains("Britain")){
			if(m_britain_current_stage <= stage){ //Check to make sure we actually need to unlock the next stage.
				m_britain_current_stage++;
			}
		}
		if(levelname.Contains("Stalingrad")){
			if(m_stalingrad_current_stage <= stage){ //Check to make sure we actually need to unlock the next stage.
				m_stalingrad_current_stage++;
			}
		}
		if(levelname.Contains("Kursk")){
			if(m_kursk_current_stage <= stage){ //Check to make sure we actually need to unlock the next stage.
				m_kursk_current_stage++;
			}
		}
		if(levelname.Contains("Normandy")){
			if(m_normandy_current_stage <= stage){ //Check to make sure we actually need to unlock the next stage.
				m_normandy_current_stage++;
			}
		}
	}
	...
	public void ResetProgress(){
		//Init(0x003FFFFF, 0x003FFFFF, 0x003FFFFF, 0x003FFFFF);
		Init(0, 0, 0, 0);
		SaveLoadManager.s_inst.SaveFile();
		//gameObject.GetComponent<KiiBucketManager>().SaveLevelInfo(0,0,0,0);
	}	
	...
	public void ShowNormandy(){
		//m_normandy_button.GetComponent<TweenColor>().PlayForward();
		m_normandy_button.transform.GetChild(0).gameObject.SetActive(true);
		m_normandy_button.transform.GetChild(0).GetComponent<Collider>().enabled = true;
		m_normandy_button.transform.GetChild(0).GetComponent<UIButton>().isEnabled = true;
		if(GameObject.Find("NormandyPlaceholder") != null)
			GameObject.Find("NormandyPlaceholder").SetActive(false);
	}
