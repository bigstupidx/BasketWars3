...
public class StoreKitEventListener : MonoBehaviour
{
	...
	void Start(){
		m_shop = gameObject.GetComponent<ShopMenu>();
	}
	...
	void transactionUpdatedEvent( StoreKitTransaction transaction )
	{
		...
		if(transaction.transactionIdentifier != "" && transaction.transactionIdentifier != null){
			m_shop.AddTransaction(transaction);
		}
	}
	...
	void purchaseFailedEvent( string error )
	{
		...
		GameManager.s_Inst.GetComponent<ShopMenu>().m_message_panel.GetComponent<TweenPosition>().PlayReverse();
		m_shop.PurchaseFailed();
	}
	...
	void purchaseSuccessfulEvent( StoreKitTransaction transaction )
	{
		...
		m_shop.ConfirmPurchase(transaction);
	}
